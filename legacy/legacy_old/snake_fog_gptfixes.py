...
def compute_visible_state(snake, food, direction):
    ...
    return normalize(visible_cells), visible_cells



def normalize(visible_cells):
    """
    Преобразует словарь visible_cells в матрицу, где для каждого пикселя вычисляется
    сумма значений R, G и B, нормализованная в диапазоне [0, 1].
    
    Параметры:
      visible_cells: dict, где ключи — кортежи (disp_col, disp_row),
                     а значения — кортежи (R, G, B) с целочисленными значениями 0–255.
                     
    Возвращает:
      numpy- вектро shape (n_rows * n_cols) с нормализованными суммами цветов.
    """
    # Определяем размеры матрицы
    max_col = max(pos[0] for pos in visible_cells.keys()) + 1
    max_row = max(pos[1] for pos in visible_cells.keys()) + 1
    
    # Инициализируем матрицу для хранения нормализованных сумм
    grid = np.zeros((max_row, max_col), dtype=float)
    
    # Для каждой позиции вычисляем сумму цветов и нормализуем её
    for (col, row), color in visible_cells.items():
        # Вычисляем сумму R + G + B
        color_sum = sum(color)
        # Нормализуем сумму, деля на 765 (максимальная сумма для (255, 255, 255))
        norm_sum = color_sum / 765.0
        grid[row, col] = norm_sum
...
class QLearner():
    ...
    def __init__(self, 
        ...
        # Инициализация весов перцептрона (размерность: n_features x n_actions)
        self.weights = np.random.randn(self.n_features, self.n_actions)
        self.last_state = np.zeros(self.n_features)
        self.last_action = 0
        self.gamma = gamma
    ...
    def action(self, state_features):
        self.last_action = self._egreedy_policy(state_features)
        return self.last_action

    def update(self, next_state_features, reward, done): 
        
        #Текущее предсказание Q(s, a) для выбранного действия
        q_current = np.dot(self.last_state, self.weights[:, self.last_action])
        # Вычисляем максимальное Q-значение для следующего состояния
        q_next = np.max(self.predict(next_state_features))
        # Вычисляем TD-целевое значение
        td_target = reward + self.gamma * q_next * (not done)
        td_error = td_target - q_current
        # Обновляем веса для выбранного действия
        self.weights[:, self.last_action] += self.learning_rate * td_error * self.last_state
        self.last_state = next_state_features
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            elif event.type == TIMER_QSTATE_SAVE_EVENT and 0:
