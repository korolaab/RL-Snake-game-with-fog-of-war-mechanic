...
class QLearner():
    ...
    def update(self, next_state, reward, done): 
        ...
        if done:
            ...
            print("Terminal state:", self.last_state)
...
def main():
    ...
    qlearner = QLearner(epsilon=1, learning_rate=0.1, gamma=0.95)
    ...
    while True:
        ...
        if not game_over:
            ...
            if new_head in snake:
                print(f"{score=}")
                print("Terminal reward: -50")
                qlearner.update(visible_state, -50, True)
                game_over = True
                ...
                if ate_food:
                    ...
                    reward = 10 - 0.1
                    qlearner.update(visible_state, reward, False)
                    food = random_food_position(snake)
                else:
                    # Small time penalty.
                    reward = -0.1
            ...
            if pygame.time.get_ticks() - game_over_time >= 100:
                ...
                qlearner.epsilon = max(0.01, qlearner.epsilon * 0.99)
        ...
        score = len(snake) - 3
        score_text = score_font.render(f"episonde: {episode} avg: {avg_score:.2f} score: {score} eps: {qlearner.epsilon:.2f}", True, WHITE)
