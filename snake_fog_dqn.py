...
def get_visible_matrix(visible_cells):
    ...
    """
    mat = np.full((VISION_DISPLAY_ROWS, VISION_DISPLAY_COLS), -1, dtype=np.float32)
    for (col, row), color in visible_cells.items():
        if color == GREEN or color == DARKGREEN:
            mat[row, col] = 1
        elif color == RED:
            mat[row, col] = 2
        elif color == WHITE:
...
class DQNAgent:
    def __init__(self, input_shape, num_actions, device, epsilon=0.5, lr=1e-3, gamma=0.99, buffer_capacity=10000, batch_size=32):
        ...
        summary(self.policy_net, input_size=(1, 11,11))
    ...
    def train_step(self):
        ...
        states = torch.tensor(states, dtype=torch.float32).to(self.device)  # shape: (B, 11, 11)
        actions = torch.tensor(actions, dtype=torch.int64).unsqueeze(1).to(self.device)
        rewards = torch.tensor(rewards, dtype=torch.float32).to(self.device)
        next_states = torch.tensor(next_states, dtype=torch.float32).to(self.device)
        dones = torch.tensor(dones, dtype=torch.float32).to(self.device)
        
        q_values = self.policy_net(states)  # shape: (B, num_actions)
...
def main():
    ...
    dqn_agent = DQNAgent(input_shape=(VISION_DISPLAY_ROWS, VISION_DISPLAY_COLS),
                         num_actions=3,
                         device=device,
                         epsilon=0.2,
                         lr=1,
    ...
    while True:
        ...
        action = dqn_agent.select_action(state_matrix)
        ...
        dqn_agent.remember(state_matrix, action, reward, next_state_matrix, done)
        ...
        score = len(snake) - 3
        score_text = score_font.render(f"episode: {episode} avg: {avg_score:.2f} score: {score} eps: {dqn_agent.epsilon:.2f}", True, WHITE)
